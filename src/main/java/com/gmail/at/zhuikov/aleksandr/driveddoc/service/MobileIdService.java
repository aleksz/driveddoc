...
public class MobileIdService {
	...
	protected HttpClient getHttpClient() {
		return HttpClients.custom()
				.setSSLSocketFactory(new SSLConnectionSocketFactory(configureSSLContext()))
				.build();
	}

	protected SSLContext configureSSLContext() {
		try {
			return SSLContexts.custom()
					.loadKeyMaterial(loadKeyStore(),  "097850N7".toCharArray())
					.build();
